...
export default function CartPage() {
  ...
  // 1. Cart Data Fetch kora
  const fetchCart = async () => {
    try {
      const res = await fetch('/api/cart/all'); // Sob item anar API
  ...
  // 2. Quantity Update Logic (Real-time)
  const updateQuantity = async (id, action) => {
    try {
      const res = await fetch(`/api/cart/update`, {
        method: 'POST',
        body: JSON.stringify({ productId: id, action }) // action: 'inc' or 'dec'
      });
      if (res.ok) {
        fetchCart(); // Data refresh
              ...
              {cartItems.map((item) => (
                <div key={item.post._id} className="bg-white p-5 rounded-[2rem] border border-slate-100 flex gap-6 items-center shadow-sm group hover:shadow-md transition-all">
                  <div className="w-24 h-24 rounded-2xl overflow-hidden bg-slate-100 shrink-0">
                    <img src={item.post.imgUrl} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  </div>
                  ...
                  <div className="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-100">
                    <button onClick={() => updateQuantity(item.post._id, 'dec')} className="p-2 hover:bg-white rounded-lg transition shadow-sm"><Minus size={16} /></button>
            ...
            <div className="lg:col-span-1">
              <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl sticky top-24">
                ...
                <div className="flex justify-between items-end mb-8">
                  ...
                  <span className="text-4xl font-black text-orange-500">${subtotal}</span>
                </div>
